				    ...
				    LocalError& error)  const
{
  ...
  Point = loctemp2;
  //error = roll.localError((cluster.firstStrip()+cluster.lastStrip())/2., 1.);
//*****
//***  error = roll.localError((cluster.firstStrip()+cluster.lastStrip())/2., cluster.clusterSize());

//std::cout<<"cs: "<<cluster.clusterSize()<<"\n"<<std::endl;
//std::cout<<"f strip: "<<cluster.firstStrip()<<" l strip: "<<cluster.lastStrip()<<"\n"<<std::endl;
  error = roll.localError((cluster.firstStrip()+cluster.lastStrip())/2., 5.);
  return true;
}
